...
/**
 * Verifica JWS signature
 * @private
 */
async function verifyJWS(credential, proof, issuerDocument) {
  // Extract public key from issuer DID document
  ...
  // TODO: Implement actual JWS verification with public key
  // For now, return true as placeholder
  return true;
}
 ...
 * @param {string} issuerDID - DID issuer
 * @returns {Promise<boolean>} True se trusted
 */
export async function verifyTrustChain(issuerDID) {
  const ebsi = new EBSIClient();

  try {
    const issuerDetails = await ebsi.getIssuerDetails(issuerDID);

    // Verifica issuer è attivo
    if (!issuerDetails.active) {
      return false;
    }

    // Verifica ha attributi corretti
    // (es. può emettere EducationalCredential)
...
/**
 * Controlla revoca credenziale
 * @param {Object} credential - VC
 * @returns {Promise<boolean>} True se revocata
 */
export async function checkRevocation(credential) {
  // TODO: Implement revocation check
  // - Check EBSI revocation registry
  // - Check status list 2021
  // - Check revocation list URL
